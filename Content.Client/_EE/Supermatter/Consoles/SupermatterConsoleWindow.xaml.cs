...
public sealed partial class SupermatterConsoleWindow : FancyWindow
{
    ...
    private bool _pendingFocusChange;
    ...
    public void UpdateUI(SupermatterConsoleEntry[] supermatters, SupermatterFocusData? focusData)
    {
        ...
        if (_pendingFocusChange)
        {
            ...
            {
                // Keep requesting the new focus until the server echoes it back.
                SendFocusChangeMessageAction?.Invoke(_trackedEntity);
            }
        }
        else if (_trackedEntity != focusData?.NetEntity)
        {
            // No pending local click: trust server focus updates.
            _trackedEntity = focusData?.NetEntity;
        }

        // Retain supermatter data for use inbetween updates
        _supermatters = supermatters;

        // Drop focus if the tracked supermatter no longer exists in this console's list.
        if (_trackedEntity != null && !_supermatters.Any(x => x.NetEntity == _trackedEntity))
        {
            _trackedEntity = null;
        ...
        // Auto-select the first supermatter to ensure details are visible by default.
        if (!_pendingFocusChange && _trackedEntity == null && _supermatters.Length > 0)
        {
            _trackedEntity = _supermatters[0].NetEntity;
            _pendingFocusChange = true;
    ...
    private void UpdateUIEntry(SupermatterConsoleEntry entry, int index, Control table, SupermatterConsoleComponent console, SupermatterFocusData? focusData = null)
    {
        ...
        if (index >= table.ChildCount)
        {
            ...
            newEntryContainer.FocusButton.OnButtonUp += args =>
            {
                ...
                _pendingFocusChange = true;
                _autoScrollAwaitsUpdate = _trackedEntity != null;
