...
public sealed partial class HFRConsoleWindow : FancyWindow
{
    ...
    private VectorFont LoadFontWithFallback(string primaryPath, string fallbackPath, int size)
    {
        if (_resourceCache.TryGetResource<FontResource>(primaryPath, out var primary))
            return new VectorFont(primary, size);

        if (_resourceCache.TryGetResource<FontResource>(fallbackPath, out var fallback))
            return new VectorFont(fallback, size);

        return new VectorFont(_resourceCache.GetResource<FontResource>("/Fonts/NotoSans/NotoSans-Regular.ttf"), size);
    }
    ...
    public void DirtyUI()
    {
        ...
        EnergyLabel.Text = $"{energyValue:0} {energyUnit}";
