...
public sealed partial class PluoxiumRadiationProductionReaction : IGasReactionEffect
{
    ...
    public ReactionResult React(GasMixture mixture, IGasMixtureHolder? holder, AtmosphereSystem atmosphereSystem, float heatScale)
    {
        ...
        else if (holder is TileAtmosphere tile)
        {
            ...
            if (entityManager.TryGetComponent<MapGridComponent>(gridUid, out var _gridComp) &&
                entityManager.TryGetComponent<GridAtmosphereComponent>(gridUid, out var _gridAtmos))
            {
                // Monolith's RadiationSystem does not expose coordinate sampling.
                // Fall back to the grid entity's receiver value when available.
                radiationLevel = GetRadiationLevel(gridUid);
            }
        }
