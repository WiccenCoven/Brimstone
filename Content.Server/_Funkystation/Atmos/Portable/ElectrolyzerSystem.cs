...
public sealed class ElectrolyzerSystem : EntitySystem
{
    ...
    private void OnActivate(EntityUid uid, ElectrolyzerComponent electrolyzer, ActivateInWorldEvent args)
    {
        ...
        if (!TryComp<ApcPowerReceiverComponent>(uid, out _))
            return;
    ...
    private void OnDeviceUpdated(EntityUid uid, ElectrolyzerComponent electrolyzer, ref AtmosDeviceUpdateEvent args)
    {
        ...
        if (initH2O > 0.05f)
        {
            ...
            var temperatureEfficiency = Math.Min(mixture.Temperature / 1123.15f * 0.8f, 0.8f);
            ...
            powerLoad = Math.Max(activeLoad * (hydrogenProduced / (maxProportion * 2f)), powerLoad);
        }
        ...
        if (initBZ > 0.01f)
        {
            var proportion = Math.Min(initBZ * (1f - MathF.Pow(MathF.E, -0.5f * temperature * workingPower / Atmospherics.FireMinimumTemperatureToExist)), initBZ);
            ...
            powerLoad = Math.Max(activeLoad * Math.Min(proportion / 30f, 1f), powerLoad);
        }
