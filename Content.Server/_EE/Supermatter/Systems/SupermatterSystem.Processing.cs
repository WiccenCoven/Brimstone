...
public sealed partial class SupermatterSystem
{
    ...
    private void ProcessAtmos(EntityUid uid, SupermatterComponent sm)
    {
        ...
        {
            var co2PP = gasReleased.Pressure * ((mix.GetMoles(Gas.CarbonDioxide) / mix.TotalMoles) * 100);
            var co2Ratio = Math.Clamp(0.5f * (co2PP - (101.325f*0.01f)) / (co2PP + (101.325f*0.25f)), 0, 1);
            var consumedCO2 = gasReleased.GetMoles(Gas.CarbonDioxide) * co2Ratio;
            consumedCO2 = Math.Min(consumedCO2, Math.Min(gasReleased.GetMoles(Gas.Oxygen), gasReleased.GetMoles(Gas.CarbonDioxide)));
        ...
        if (mix.GetMoles(Gas.AntiNoblium) > 0.01f && mix.GetMoles(Gas.Helium) > 0.01f)
        {
            ...
            if (consumedAN > 0)
            {
                ...
                {
                    _lightning.ShootRandomLightnings(uid, zapRange, zapCount, sm.LightningPrototypes[zapPower]); sm.ZapLast = _SmTiming.CurTime;
                }
            }
        }
    ...
    private void AnnounceCoreDamage(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (_SmTiming.CurTime < sm.YellLast + sm.YellTimer)
            return;
        ...
        if (sm.Delamming && sm.DelamAnnounced)
        {
            var seconds = (int)Math.Ceiling(sm.DelamEndTime.TotalSeconds - _SmTiming.CurTime.TotalSeconds);
    ...
    private void HandleDelamination(EntityUid uid, SupermatterComponent sm)
    {
        ...
        {
            sm.Delamming = true;
            sm.DelamEndTime = _SmTiming.CurTime + TimeSpan.FromSeconds(sm.DelamTimer);
        ...
        if (_SmTiming.CurTime < sm.DelamEndTime)
            return;
    ...
    {
        if (sm.AccentLastTime >= _SmTiming.CurTime || !_random.Prob(0.05f))
            return;
        ...
        if (sm.AccentLastTime + TimeSpan.FromSeconds(nextSound) > _SmTiming.CurTime)
            return;

        if (sm.Status >= SupermatterStatusType.Danger)
            sound = sm.DelamAccent;

        sm.AccentLastTime = _SmTiming.CurTime;
